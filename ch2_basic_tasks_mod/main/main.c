...
task_loop (void * argp)
{
    ...
    for (;;)
    {
        ...
        fprintf(stderr, "Suspending middle LED task\n");
        vTaskSuspend(p_led->taskh);
        vTaskDelay(pdMS_TO_TICKS(5000));
        fprintf(stderr, "Resuming middle LED task\n");
        vTaskResume(p_led->taskh);
    }
}
...
app_main (void)
{
    ...
    xTaskCreate(task_loop, "task_loop", 1024, &g_leds[1], 1, NULL);
